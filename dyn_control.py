...
def cf_velocity(v_k1, alpha, inference):
    ...
    return v_k
#v_ka1 =0 
#v_ka=(1-alpha)*v_ka1 + alpha*(1-inference_s) * VEL_MAX
...
def cf_controller():
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        while(steps <= 250):
            ...
            inference_s = (cf_conn1.recv() / 5)
            yawrate = (inference_s * 30) #steering_angle inference_s #18
            #print("angle = %s and yawrate = %s" % (inference_s, yawrate))
            v_k = cf_velocity(v_k1, ALPHA, cf_conn.recv()/5)
            cf.commander.send_hover_setpoint(v_k, 0, yawrate, 0.8) #0
            v_k1 = v_k
            steps += 1
            time.sleep(0.1)

...
# double ended queue to store steering angle values
class InfQueue_s():
    ''' manages sum of queue for rolling average '''
    def __init__(self):
        #self.pred_queue = deque()
    ...
    def enqueue(self, steer):
        if (len(self.steer_queue) == 0):
            #if(len(self.steer_queue == 1 ))
            #steer_queue[0] = steer_queue[0]*0.7

            self.steer_queue.append(steer)
            #steer_queue[1] = steer_queue[1]*0.5
            self.sum2 += steer
        else:
            self.sum2 -= self.steer_queue.popleft()
            self.steer_queue.append(steer)
            self.sum2 += steer
            cf_tx1.send(self.sum2) 
...
class FrameViewer(Gtk.Window):
    ...
    def _showframe(self, imgdata):
        ...
        if (self._start != None):
            ...
            GLib.idle_add(self.set_title, "{:.1f} fps / {:.1f} kb / {:.1f} prediction / {:.1f} angle".format(fps, len(imgdata)/1000, (inference.sum/5),(inference_s.sum2)))
        ...
        try:
            ...
            if (pix != None):
                ...
                #Model = numpy.asarray(model(image))
                angle , predic = model(image) #.numpy()[0,0]

                prediction = predic.numpy()[0,0]
                steering_angle = angle.numpy()[0,0]

                # generate inference from model 
                #prediction  = model(image).numpy()[0,0]

                # steering angle calculation
                #steering_angle = steering_model(image).numpy()[0,0]

                # append inference data to queue
                inference.enqueue(prediction)
                inference_s.enqueue(steering_angle)
...
if __name__ == '__main__':
    ...
    cf_tx1 = Client(('localhost', 7000), authkey=b'crazyflie')
    ...
    #steering_model = load_model("steering_model.h5")
